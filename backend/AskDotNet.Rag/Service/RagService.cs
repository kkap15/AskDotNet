...
public sealed class RagService(
    ...
    : IRagService, IAsyncDisposable
{
    private readonly DatabaseHelper _dbHelper = new(connectionString);

    public async Task<RagResponse> AskAsync(string question, CancellationToken ct = default)
    {
        logger.LogInformation("Processing question: {Question}", question);
        var (chunks, source) = await PrepareAsync(question, 10, ct);

        if (chunks.Count == 0)
        {
            logger.LogWarning("No chunks found for question: {Question}", question);
        ...
        var answer = await RagHelper.GenerateAnswerAsync(question, context, chatClient, ct);
    ...
    {
        var (chunks, sources) = await PrepareAsync(question, 10, cancellationToken);
    ...
    {
        var (_, sources) = await PrepareAsync(question, topK, cancellationToken);

        return sources;
    }

    private async Task<(List<(string SourceUrl, string SourceTitle, string SectionHeading, string Content, 
            double Similarity)> Chunks, List<ChunkReference> Sources)> PrepareAsync(string question, int topK, CancellationToken ct)
    {
        var questionEmbeddings = await generator.GenerateAsync(new List<string> { question }, cancellationToken: ct);
